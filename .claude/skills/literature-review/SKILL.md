...
** MANDATORY: Every literature review MUST include at least 1-2 AI-generated figures using the scientific-schematics skill.**
...
1. **Multi-Database Search**:
   ...
   **Web-Based Academic Search (parallel-web skill ¡ª START HERE):**
...
5. **Create PRISMA Flow Diagram**:
   ```
   Initial search: n = X
   ©À©¤ After deduplication: n = Y
   ©À©¤ After title screening: n = Z
   ©À©¤ After abstract screening: n = A
...
2. **Use multiple databases** (minimum 3): Ensures comprehensive coverage ¡ª parallel-web counts as one source
3. **Include preprint servers**: Captures latest unpublished findings
4. **Document everything**: Search strings, dates, result counts for reproducibility ¡ª save all parallel-cli output to `sources/`
...
2. **Screen systematically**: Title ¡ú Abstract ¡ú Full text
...
### Web Search & Extraction (parallel-web skill ¡ª PRIMARY)
- **parallel-cli search**: Broad academic and general web search with domain filtering ¡ª use for initial scoping, finding papers, citation chaining, and supplementary searches
- **parallel-cli extract**: Fetch full content from paper URLs, journal websites, and preprint servers ¡ª use for reading abstracts, extracting reference lists, and verifying paper details
...
### Required CLI Tools
```bash
# parallel-cli (PRIMARY ¡ª for web search and URL extraction)
