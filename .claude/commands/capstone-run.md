...
1. **预检**（无 Bash 调用，先看文件）：
   ...
   - 如果 `capstone-科研数据分析/data/raw/era5_all.csv` 已存在且 ≥ 70000 行，告诉学员"已有 30 年数据，是否复用？跳过 01 直接跑 02？"

2. **第 1 步：下载**
   - 跑 `python capstone-科研数据分析/01_下载.py`
   - 默认 30 年，约 1-2 分钟
   - **失败常见原因**：
     - 网络超时 → 提示学员切换 CDS 路径（`01_下载_cds.py`），或直接看 `example/capstone_5y_cds/data/` 里的 CDS 参考 CSV
...
**不要**自己改脚本——只跑、只解释。
**不要**重复跑——除非学员明确要求。
**遇到 7 城都连不上**：告诉学员切到 CDS 路径（`01_下载_cds.py`），或从 `example/capstone_5y_cds/data/` 拷 CDS 参考 CSV 当输入跳过下载（注意 CDS 是月数据，要跑 `02_分析_cds.py` 不是 `02_分析.py`）。
