---
description: 用户粘报错或失败的命令，给出原因、下一步、是否可跳过
---

# /explain-error — 报错翻译官

用户会在 `$ARGUMENTS` 里粘一段报错（或一段失败命令的输出）。你的任务是：
...
3. 如果是网络类、Docker 类、Ollama 类 → 提醒"先看 example/ 文件夹里的参考产物，能不能用现成结果继续"。

4. 如果用户粘的是英文长 stack trace，**不要**整段翻译——抓**最后那行错误的核心**（`xxxError: ...` 部分），其它略过。

5. 如果你也不确定原因，**直接说"我不确定"**，然后建议把这一段贴到课程 Issue。**不要瞎猜。**
