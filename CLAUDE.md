...
PPT 不进仓库，由讲师课堂提供。各章 README 顶部的"PPT 页码"只用于现场对齐。
...
pipeline_template/                                跨学科 4 步骨架（学员套自己方向）
...
- 课堂主线 = 三角色（资料员 → 起草员 → 审稿员合并 critic + polish）→ `capstone/04_起草.md`
...
- `07-MCP扩展/mcp配置样例.json` 里 `_README` / `_说明` 字段不是合法 MCP 字段，是行内教学注释——学员拷到自己 `~/.claude.json` 时要去掉。
- 价格表只作教学估算，每个供应商带官网链接，**讲师上课前过一遍官网**重查。
